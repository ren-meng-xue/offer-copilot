...
from backend.app.schemas.knowledge import CreateKnowledgeResponse, KnowledgeBaseListItem, KnowledgeStatusResponse
...
# 查询知识库状态，返回当前进度和错误信息。
async def get_knowledge_status(db: AsyncSession, kb_id: int, user_id: int) -> KnowledgeStatusResponse | None:
...
async def list_knowledge_bases(db: AsyncSession, user_id: int) -> list[KnowledgeBaseListItem]:
    """返回当前用户创建的知识库列表，供前端知识库页面展示。"""

    knowledge_bases = await knowledge_repository.list_knowledge_bases_by_user(db, user_id)
    return [
...
async def delete_knowledge_base(db: AsyncSession, kb_id: int, user_id: int) -> None:
    ...
    if kb.status in {KnowledgeBaseStatus.PENDING, KnowledgeBaseStatus.PROCESSING}:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Knowledge base is still processing")
...
async def update_knowledge_status(
    ...
    await knowledge_repository.update_knowledge_base_status(db, kb_id, status, error_message)

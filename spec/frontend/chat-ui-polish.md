...
- 问题一旦成功进入 optimistic message 阶段，输入框必须立即清空，不等待 SSE `done`。
...
- `/chat` 草稿态顶部展示知识库下拉选择。
- 首问发送并跳转到 `/chat/[conversationId]` 后，当前会话仍需展示只读知识库上下文，避免用户误以为知识库已丢失。
- 已有会话页只展示当前知识库信息，不提供会话内切换知识库能力。
- 首问跳转过程应尽量平滑，不应因为路由切换出现明显“内容闪没一下”的感知。
...
flowchart TD
  ...
  D --> E[选择知识库并发送第一条问题]
  E --> F[创建后端会话]
  F --> G[插入 optimistic messages 并立即清空输入框]
  G --> H[进入 /chat/conv_id 并流式回答]
  H --> I[会话页展示只读知识库上下文]

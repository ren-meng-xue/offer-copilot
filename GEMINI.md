# Job Intel Agent - Gemini 主控规范

本文件是从项目 `CLAUDE.md` 与 `.claude/` 配置同步给 Gemini 的项目级规则。Gemini CLI 约定读取 `GEMINI.md`，因此本文件是项目内 Gemini 的入口规范。
...
遇到以下情境时，先主动读取对应 `.claude/skills/*.md` 文件，并按其中流程执行。Claude 专用的 `superpowers:*`、`gstack:*`、MCP 配置在 Gemini 中不可直接等价时，使用 Gemini 可用工具或手工流程替代，并说明替代方式。
...
| 代码审查（任意场景） | 按代码审查模式执行，优先列问题与风险 |
| 并行子任务开发（多模块同步） | 仅在用户明确要求并行 agent 时使用对应能力 |
